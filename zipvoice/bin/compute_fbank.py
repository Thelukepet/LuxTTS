...
def compute_fbank_split_single(params, idx):
    logging.info(f"Computing features for {idx}-th split of {params.dataset} dataset {params.subset} subset")
...
def compute_fbank_split(params):
    if params.split_end < params.split_begin:
        logging.warning(
            f"Split begin should be smaller than split end, given {params.split_begin} -> {params.split_end}."
        )
...
def compute_fbank(params):
    logging.info(f"Computing features for {params.dataset} dataset {params.subset} subset")
    src_dir = Path(params.source_dir)
    output_dir = Path(params.dest_dir)
