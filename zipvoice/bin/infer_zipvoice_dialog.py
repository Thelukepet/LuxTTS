...
def generate_sentence(
    ...
    sequential_chunked_wavs = [
        sequential_indexed_chunked_wavs[i][1] for i in range(len(sequential_indexed_chunked_wavs))
    ]
...
def generate_sentence_stereo_raw_evaluation(
    ...
    if len(loaded_prompt_wavs) == 1:
        assert loaded_prompt_wavs[0].size(0) == 2, "Merged prompt wav must be stereo for stereo dialogue generation"
        prompt_wav = loaded_prompt_wavs[0]
...
def generate_list(
    ...
    logging.info(f"Average RTF w/o vocoder: {np.sum(total_t_no_vocoder) / np.sum(total_wav_seconds):.4f}")
    logging.info(f"Average RTF vocoder: {np.sum(total_t_vocoder) / np.sum(total_wav_seconds):.4f}")
...
def main():
    ...
    assert params.test_list is not None, "For inference, please provide prompts and text with '--test-list'"
    ...
    if params.model_dir is not None:
        ...
        logging.info(
            f"Using {params.model_name} in local model dir {params.model_dir}, checkpoint {params.checkpoint_name}"
        )
    else:
