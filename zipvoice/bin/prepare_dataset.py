    ...
    with open(tsv_path, "r") as fr:
        for line in fr:
            ...
            if len(items) == 3:
                ...
                raise ValueError(f"Invalid line format: {line},requries to be 3 columns or 5 columns")
    ...
    with ThreadPoolExecutor(max_workers=num_jobs) as ex:
        ...
        for future in tqdm(futures, desc="Processing recordings"):
            try:
                ...
                logging.warning(f"Error processing recording {recording_id} with error: {e}")
