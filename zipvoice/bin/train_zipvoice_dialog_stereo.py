...
def get_parser():
    ...
    parser.add_argument("--base-lr", type=float, default=0.002, help="The base learning rate.")
...
def compute_fbank_loss(
    ...
    assert features.size(2) == 3 * params.feat_dim, (
...
def run(rank, world_size, args):
    ...
    if not hasattr(train_cuts[0].supervisions[0], "tokens") or not hasattr(dev_cuts[0].supervisions[0], "tokens"):
        logging.warning(
            "Tokens are not prepared, will tokenize on-the-fly, which can slow down training significantly."
        )
