...
class ModelPool:
    ...
    async def acquire(self, timeout: int = MODEL_ACQUIRE_TIMEOUT) -> ModelInstance:
        ...
        try:
            ...
            instance = await asyncio.wait_for(self.queue.get(), timeout=timeout)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for inst in self.instances:
            avg_time = (
                inst.stats.total_inference_time / inst.stats.total_requests if inst.stats.total_requests > 0 else 0
            )
...
def load_speaker_embeddings(model: Any, speakers_info: Dict[str, SpeakerInfo]):
    ...
    for key, speaker_info in speakers_info.items():
        try:
            ...
            # 传递配置文件中指定的参考文本
            ref_embedding = model.encode_prompt(
                speaker_info.wav_path, duration=ref_duration, rms=rms, text=speaker_info.text
            )
...
app = FastAPI(title="Lux-TTS Server", description="Lux-TTS API Service", lifespan=lifespan)
...
def initialize_resamplers():
    ...
    try:
        ...
        gpu_resampler = torchaudio.transforms.Resample(
            orig_freq=ORIGINAL_SAMPLE_RATE, new_freq=TARGET_SAMPLE_RATE, dtype=torch.float32
        ).to("cuda")
...
async def root():
    """根路径，返回API信息"""
    return {"message": "Welcome to Lux-TTS Server", "endpoints": {"http": "/tts/http"}}
...
async def health_check():
    ...
    try:
        if not hasattr(app.state, "model_pool"):
            return JSONResponse(
                status_code=503, content={"status": "unhealthy", "message": "Model pool not initialized"}
            )
        ...
        return JSONResponse(status_code=503, content={"status": "unhealthy", "message": str(e)})
