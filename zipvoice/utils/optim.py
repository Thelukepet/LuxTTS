...
class ScaledAdam(BatchedOptimizer):
    ...
    def _get_clipping_scale(self, group: dict, tuples: List[Tuple[Tensor, dict, List[str]]]) -> float:
        ...
        if ans < 0.5:
            logging.debug(f"Scaling gradients by {ans}, model_norm_threshold={model_norm_threshold}")
    ...
    def _show_param_with_unusual_grad(
        ...
        for p, state, batch_param_names in tuples:
            ...
            grad_ratio = (mean(p.grad**2) / state["exp_avg_sq"].mean(dim=dims)).sqrt().to("cpu")

            ratios_names += zip(grad_ratio.tolist(), batch_param_names, p.grad.unbind(dim=0))
        ...
        logging.debug(f"Parameters with most larger-than-usual grads, with ratios, are: {ratios_names}")
